    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("GET".equalsIgnoreCase(method)) {
            ...
            try {
                ...
                System.out.println("Error at CreateProductController method get with ERROR: "+e.toString());
            } finally {
            ...
            try {
                ...
                List<Category> listCategory = pdao.GetAllCategory();
                if (category != null) {
                    Product p = new Product(0, name, price, quantity, description, img, category);
                    Boolean check = pdao.InsertProduct(p);
                    if (check) {
                        url = SUCCESS;
                        request.setAttribute("listCategory", listCategory);
                    } else {
                        request.setAttribute("ERROR", "Create Product Fail !!");
                    }
                }
            } catch (Exception e) {
                System.out.println("Error at CreateProductController with ERROR: " + e.toString());
                request.setAttribute("ERROR", "Create Product Fail at CreateProductController !!");
            } finally {
                request.getRequestDispatcher(url).forward(request, response);
            }
        }
